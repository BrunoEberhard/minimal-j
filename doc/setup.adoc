...
with the ajax protocol but with WebSockets. As you see in the class the configuration of the WebSocket server is done by annotation. You don't
need to add another servlet in the web.xml. But you need some additional libraries. Remember: this applies only if you want to run your application
with WebSocket. The ajax protocol will runt out of the box.