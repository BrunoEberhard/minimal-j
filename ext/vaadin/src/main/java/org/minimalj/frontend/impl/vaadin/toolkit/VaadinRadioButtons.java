...
public class VaadinRadioButtons<T> extends RadioButtonGroup<T> implements Input<T>, HasCaption {
	private static final long serialVersionUID = 1L;
	
	private final List<T> objects;
	private T setObject;
	
	public VaadinRadioButtons(List<T> objects, InputComponentListener listener) {
		// setNullSelectionAllowed(true);
		this.objects = objects;
		updateChoice();
		addValueChangeListener(event -> listener.changed(VaadinRadioButtons.this));
