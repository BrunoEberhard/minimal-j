...
type parameter /and/ remove the createObject the default createObject will throw an exception!
...
Note1: First the fancy object was called _ but this is not recommanded to be used in java 8 anymore.

Note2: Do *never* change the content of the $ object. Not even temporary. You'll see other possibilities
to change the created form fields.

Note3: The $ trick works quite well. There is only one point where it gets ugly: enum. Don't be
